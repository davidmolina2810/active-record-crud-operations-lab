...
def can_be_created_in_a_block(args = nil)
  ...
  # release_date == 1990
  if args
    create_movie(args[:title], args[:release_date])
  else 
    create_movie
  end
end

def create_movie(title = "Home Alone", release_date = 1990)
  Movie.create do |m|
    m.title = title
...
def can_find_using_where_clause_and_be_sorted
  ...
  Movie.all.where('movies.release_date > 2002').sort.reverse
end
...
def can_update_using_update_method
  ...
  movie_id = Movie.all.find_by(title: "Wat?").id 
